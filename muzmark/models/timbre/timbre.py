...
from . import hifigan
from .blocks import Conv2Encoder, FCBlock, ReluBlock, WatermarkEmbedder, WatermarkExtracter
...
def get_vocoder(vocoder_config, vocoder_checkpoint, device):
    with open(vocoder_config, "r") as f:
        config = json.load(f)
    config = hifigan.AttrDict(config)
    vocoder = hifigan.Generator(config)
    ckpt = torch.load(vocoder_checkpoint, map_location=device, weights_only=True)
...
class Encoder(nn.Module):
    def __init__(self, process_config, model_config, msg_length, device="cuda:0"):
        ...
        #stft transform
        self.stft = fixed_STFT(process_config["mel"]["n_fft"], process_config["mel"]["hop_length"], process_config["mel"]["win_length"], device=device)
    ...
    def test_forward(self, x, msg, ratio = 1.0):
        num_samples = x.shape[2]
        spect, phase = self.stft.transform(x)
        ...
        ###############################################################################################################
        # 我加的调整水印强度的一个东西。
        watermark = carrier_watermarked - spect.unsqueeze(1)
        if ratio != 1.0:
            carrier_watermarked = spect.unsqueeze(1) + ratio * watermark
        ###############################################################################################################
        
        self.stft.num_samples = num_samples
        y = self.stft.inverse(carrier_watermarked.squeeze(1), phase.squeeze(1))
        return y, carrier_watermarked, spect.unsqueeze(1), watermark
...
class Decoder(nn.Module):
    def __init__(self, process_config, model_config, msg_length, vocoder_config=None, vocoder_checkpoint=None, device="cuda:0"):
        super(Decoder, self).__init__()
        self.robust = model_config["robust"]
        if self.robust:
            self.dl = distortion()

        self.mel_transform = TacotronSTFT(filter_length=process_config["mel"]["n_fft"], hop_length=process_config["mel"]["hop_length"], win_length=process_config["mel"]["win_length"], device=device)
        # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.vocoder = get_vocoder(vocoder_config, vocoder_checkpoint, device=device)
        self.vocoder_step = model_config["structure"]["vocoder_step"]
...
class Discriminator(nn.Module):
    def __init__(self, process_config, device="cuda:0"):
